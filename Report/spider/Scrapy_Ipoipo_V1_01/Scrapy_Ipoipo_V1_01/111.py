...
def get_content():
    ...
    for n in range(1, 31):
        ...
        for con in content:
            ...
            title = ht.xpath('//div[@class="con main"]/p[2]/a/text()')[0]
            print(title)
            d.find_element_by_xpath('//div[@class="con main"]/p[2]/a').click()
            time.sleep(20)

            rename = un_zip(download_dir, 'D:\\报告\\文件', title + '.pdf', CATES_DICT)
            time.sleep(1)
            try:
                # 文章具体路径
